...
void ResourceBrowser::constructFrame(bool independent)
{
    ...
    for (auto it = resource_map.begin(); it != resource_map.end(); it++)
    {
        ...
        if (*(m_selected_resource[it->second->getResourceId()]))
        {
            if (m_resource_views.find(std::type_index(typeid(*(it->second)))) != m_resource_views.end())
            {
                m_resource_views[std::type_index(typeid(*(it->second)))]->setResource(it->second);
                m_resource_views[std::type_index(typeid(*(it->second)))]->constructFrame(false);
            }
            else
            {
                ResourceView rv;
...
void ResourceBrowser::addResourceView(std::type_index type, std::unique_ptr<ResourceView> view)
{
